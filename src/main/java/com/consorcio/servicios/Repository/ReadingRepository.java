...
public interface ReadingRepository extends JpaRepository<Reading, Long> {
    ...
    @Query("SELECT r FROM Reading r WHERE r.idMeter = (SELECT res.idMeter FROM Residence res WHERE res.idUser = :idUser) AND r.idPeriod = :idPeriod ORDER BY r.dateRegister DESC LIMIT 1")
    ...
    @Query("SELECT r FROM Reading r "
            ...
            + "ORDER BY r.dateRegister DESC LIMIT 1")
